...
let assemble (inss : sinstr list) : int list =
    let aux =
        ...
        | SPop -> [5]
        | SSwap -> [6]
    List.fold (fun acc instr -> acc @ aux instr) [] inss

let compile (e : expr) (cenv : stackvalue list) = assemble (scomp e cenv)

let si1 = [SCstI 17; SVar 0; SVar 1; SAdd ; SSwap; SPop]

let si1r = [0; 17; 1; 0; 1; 1; 2; 6; 5]

printfn "%A => %A - expected result = %A" si1 (assemble si1) si1r

printfn "%A => %A" e1 (compile e1 [])
