...
let rec cStmt stmt (varEnv: varEnv) (funEnv: funEnv) : instr list =
    ...
    | Expr e ->
        cExpr e varEnv funEnv
        @ [ INCSP -1 ] (* Remove result of expression from stack, as this is a statement *)
    | Block stmts ->
        ...
        code
        @ [ INCSP(snd varEnv - fdepthend) ] (* Remove variables, declared in the block, from the stack *)
    | Return None -> [ RET(snd varEnv - 1) ]
    | Return(Some e) -> cExpr e varEnv funEnv @ [ RET(snd varEnv) ]
    | Switch(e, cases) ->
        let rec loop cases label =
            match cases with
            | [] -> [Label label]
            | Case(csti, stmt) :: tail ->
                let hereLabel = newLabel ()

                [ Label label; DUP; CSTI csti; SUB; IFNZRO hereLabel ]
                @ cStmt stmt varEnv funEnv
                @ loop tail hereLabel

        cExpr e varEnv funEnv @ loop cases (newLabel ()) @ [INCSP -1]
...
and cExpr (e: expr) (varEnv: varEnv) (funEnv: funEnv) : instr list =
    ...
    // Exercise 8.5
    | Ternary(e1, e2, e3) -> cExpr (Orelse(Andalso(e1, e2), e3)) varEnv funEnv
...
let cProgram (Prog topdecs) : instr list =
    ...
    let compilefun (tyOpt, f, xs, body) =
        ...
        [ Label labf ]
        @ code
        @ [ RET(List.length paras - 1) ] (* -1 because there is no result value on the stack; we leave a dummy one - body of function is always a statement. *)
